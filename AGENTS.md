...
CI / deploy: [`.github/workflows/ci.yml`](.github/workflows/ci.yml) (audit `--audit-level=high`, lint, typecheck, i18n, resume keys, build on PR/push; semantic-release on `main` push via GitHub App token — `RELEASE_APP_CLIENT_ID` / `RELEASE_APP_PRIVATE_KEY`), [`.github/dependabot.yml`](.github/dependabot.yml) (weekly grouped PRs: npm production-patch / production-minor / development, Actions patch / minor; major individual), [`.github/workflows/dependabot-automerge.yml`](.github/workflows/dependabot-automerge.yml) (auto-merge `production-patch`, `development`, Actions `patch`), [`vercel.json`](vercel.json) (pnpm install/build for Vercel Git deploys).
